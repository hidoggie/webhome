...
function routeTo(pageId) {
    ...
    if (pageId === 'intro') {
        ...
        document.querySelectorAll('#global-bottom-nav .nav-item').forEach(item => item.classList.remove('active'));
        if(pageId === 'map') document.querySelectorAll('#global-bottom-nav .nav-item')[0].classList.add('active');
        if(pageId === 'mission') document.querySelectorAll('#global-bottom-nav .nav-item')[1].classList.add('active');
...
function initNaverMap() {
    var koreaBounds = new naver.maps.LatLngBounds(
        ...
        new naver.maps.LatLng(38.311820, 130.256601)  
    );
    ...
    eventLocations.forEach(function(loc) {
        ...
        if (userLat && userLng) {
            ...
            if (distanceMeters <= 1000) {
                ...
                if (!isAcquired) rightSideHtml = `<span class="stamp-zone-text">스탬프존 ${Math.round(distanceMeters)}m</span>`;
            } else {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function(position) {
            renderLighthouseList(position.coords.latitude, position.coords.longitude);
        }, function(error) {
    ...
    if(btnLocation) {
        btnLocation.addEventListener('click', function() {
            ...
            if (navigator.geolocation) {
                ...
                navigator.geolocation.getCurrentPosition(function(position) {
                    var newLatLng = new naver.maps.LatLng(position.coords.latitude, position.coords.longitude);
                    renderLighthouseList(position.coords.latitude, position.coords.longitude);

                    if (!myLocationMarker) {
                        myLocationMarker = new naver.maps.Marker({
                            position: newLatLng, map: map,
                            icon: {
                                content: '<div class="my-loc-pin"><img src="img/lh-man-map.png" style="width:40px;height:40px;"></div>',
                                size: new naver.maps.Size(20, 20), anchor: new naver.maps.Point(10, 10)
                            }
                        });
                    } else {
...
function closePassportModal() { document.getElementById('passportModal').style.display = 'none'; }
function startNewJourney() { closePassportModal(); routeTo('map'); }
function resumeJourney() {
    const val = document.getElementById('existingPassportId').value.trim();
    if(!val) { alert('여권 번호를 입력해주세요.'); return; }
    alert('[' + val + '] 여권으로 이력을 불러옵니다!');
    routeTo('map'); 
}
function copyPassportId() {
    const idText = document.getElementById('newPassportId').innerText;
    navigator.clipboard.writeText(idText).then(() => alert('복사되었습니다!')).catch(() => alert('복사에 실패했습니다.'));
}
...
// 팝업 및 앱링크 관련
let currentSelectedLoc = null; let isPlaying = false;
const audio = document.getElementById('audioPlayer'); const audioBtn = document.getElementById('audioBtn');
...
function closePopupOutside(e) { if (e.target === document.getElementById('popupOverlay')) { document.getElementById('popupOverlay').style.display = 'none'; if(isPlaying) toggleAudio(); } }
function toggleAudio() {
    if (!currentSelectedLoc || !currentSelectedLoc.audio) { alert('음성 파일이 준비 중입니다.'); return; }
    if (!isPlaying) { document.getElementById('audioSource').src = currentSelectedLoc.audio; audio.load(); audio.play(); isPlaying = true; audioBtn.classList.add('playing'); }
    else { audio.pause(); audio.currentTime = 0; isPlaying = false; audioBtn.classList.remove('playing'); }
}
...
setInterval(function() {
    ...
    if(!sky || document.getElementById('page-intro').style.display === 'none') return; // 인트로 화면일때만 생성
